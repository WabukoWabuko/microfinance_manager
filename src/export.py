...
class ExportManager:
    ...
    def export_data(self, user_id, format_type):
        try:
            data = self.db.fetchall("SELECT * FROM Transactions WHERE loan_id IN (SELECT id FROM Loans WHERE user_id = ?)", (user_id,))
            if format_type == "CSV":
                with open(f"export_{user_id}.csv", "w", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow(["ID", "Loan ID", "Amount", "Type", "Created At"])
